...
public class SearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (session==null) {
			 ...
			 * out.println("<input type='submit' value='search'>"); out.println("</form>");
			 */
			
			String name =req.getParameter("name");
			if(name!=null) {
				EmployeeDAO dao = EmployeeDAOFactory.getEmployeeDAO();
				List<EmployeeBeen> beans =dao.search(name);
				...
				/*if (beans!=null && !beans.isEmpty()) {
					...
					out.println("</tr>");
					for (EmployeeBeen been : beans) {
						out.println("<tr>");
						out.println("<td>");
						out.println(been.getName());
						out.println("<td></td>");
						out.println(been.getEmail());
						out.println("</td>");
						out.println("</tr>");
					}
					out.println("</table>");
				} else {
					out.println("<h1>No Data Found for the name "+ name +"</h1>");
				}
			}
